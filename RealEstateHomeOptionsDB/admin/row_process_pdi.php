...
//a	array_push($rtn_rows, "PDICatID=$PDICatID");
	...
	//print("DefectDesc_name $i" .  $$DefectDesc_name . "<BR>");	
		if (trim($$DefectDesc_name) <> "")
		{
			...
			$PDICatID_new_name		= "PDICatID_New_Text_" . $i;
			...
			// if there is value at new Category textfield, and PDICatID == -1
			// store new Category value into PDICatID var
			if ($$PDICatID_new_name && $PDICatID == -1){
				$PDICatID = preg_replace($find, $replace, trim($$PDICatID_new_name));
			}

			if ($$PDITypeID_name)		$PDITypeID = $$PDITypeID_name;
			else $PDITypeID = 0;

			// if there is value at new PDI Defect Type textfield, and PDITypeID == -1
			// store new PDI Defect Type value into PDITypeID var
			if ($$PDITypeID_new_name && $PDITypeID == -1){
				$PDITypeID = preg_replace($find, $replace, trim($$PDITypeID_new_name));
			}			
